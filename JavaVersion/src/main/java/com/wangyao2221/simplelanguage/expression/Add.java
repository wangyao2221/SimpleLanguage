...
public class Add implements Expression<Expression> {
    ...
    public Expression reduce(Context context) {
        if (left.reducible()) {
            return new Add((Expression) left.reduce(context), right);
        } else if (right.reducible()) {
            return new Add(left, (Expression) right.reduce(context));
        } else {
